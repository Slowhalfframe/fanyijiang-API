...
class UserCreateArticle(BaseCreateContent):
    ...
    def get_user_article(self):
        ...
        articles = [a for a in Article.objects.filter(author=self.user)]
        return articles
...
class UserCreateCollect(BaseCreateContent):
    ...
    def get_user_favorites(self):
        '''获取用户的收藏夹'''
        favorites = [a for a in UserFavorites.objects.filter(user=self.user)]
        return favorites
    ...
    def get_recent_vote_content(self):
        '''获取最近点赞的内容'''
        ac_votes = Vote.objects.filter(author=self.user)
