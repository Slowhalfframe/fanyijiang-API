...
from apps.userpage_v2.models import (UserProfile, UserFavorites, FollowedUser, FollowedFavorites,
                                  FavoriteCollection)
from apps.userpage_v2.serializers import (UserInfoSerializer, FavoritesSerializer, FollowsUserSerializer,
...
from apps.userpage_v2.validators import FavoritesValidator
...
from apps.taskapp_v2.tasks import notification_handler

from apps.pins.models import Idea

from apps.creator_v2.views import ArticleVoteStatistics, ArticleCollectStatistics, AnswerVoteStatistics, \
...
class UserInfoAPIView(CustomAPIView):
    ...
    def get(self, request, user_slug):
        ...
        create_content_nums = {
            ...
            'article_count': Article.objects.filter(author_id=user.uid, is_draft=False, is_deleted=False).count(),
...
class ArticleListAPIView(CustomAPIView):
    ...
    def get(self, request, user_slug):
        ...
        # TODO 查询相关数据库
        articles = Article.objects.filter(author_id=user.uid, is_draft=False, is_deleted=False)
...
class FavoritesContentAPIView(CustomAPIView):
    ...
    def post(self, request, user_slug, pk):
        ...
        if content_type == 'article':
            article = Article.objects.filter(pk=object_id, is_draft=False, is_deleted=False).first()
...
class CollectedAPIView(CustomAPIView):
    ...
    def get(self, request, user_slug, content_type, object_id):
        ...
        if content_type == 'article':
            instance = Article.objects.filter(pk=object_id, is_draft=False, is_deleted=False).first()
        if content_type == 'think':
