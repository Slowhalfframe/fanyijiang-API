...
from apps.questions_v2.models import Answer, Question, QuestionFollow

from apps.taskapp_v2.utils import creator_list

from apps.creator_v2.models import CreatorList, SeveralIssues
...
def notification_handler(actor_uid, recipient_uid, verb, action_object_id, **kwargs):
    ...
    if verb == 'I':
        ...
        is_object = actor_uid == action_object.author_id
    elif verb == 'A':
        # 传入回答对象
        action_object = Answer.objects.get(pk=action_object_id)
        is_object = recipient_uid == action_object.question.author_id
    elif verb == 'O':
        # 传入用户对象
        ...
        # 传入评论对象, 评论回答
        action_object = Comment.objects.get(pk=action_object_id)
        obj = action_object.content_object
        is_object = recipient_uid == obj.author.pk
    elif verb == 'CQ':
        # 传入评论对象, 评论问题
        action_object = Comment.objects.get(pk=action_object_id)
        obj = action_object.content_object
        ...
        # 赞了你的回答
        action_object = Answer.objects.get(pk=action_object_id)
        is_object = recipient_uid == action_object.author_id
