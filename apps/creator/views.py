...
class BaseStatistics(object):
    ...
    def get_queryset(self):
        queryset = self.which_model.objects.filter(author_id=self.user.uid)
        return queryset
...
class AnswerVoteStatistics(BaseStatistics):
    ...
    def get_date_upvote_nums(self, date):
        ...
        end_date = begin_date + datetime.timedelta(days=1)
        queryset = self.get_queryset()
        nums_list = [query.votes.filter(value=True, create_at__gte=begin_date, create_at__lte=end_date).count() for query
                     in
                     queryset]
        nums = sum(nums_list) or 0
        return nums
...
class ArticleCommentStatistics(BaseStatistics):
    ...
    def get_date_total_comments(self, date):
        ...
        end_date = begin_date + datetime.timedelta(days=1)
        queryset = self.get_queryset()
        nums_list = [query.comments.filter(create_at__gte=begin_date, create_at__lte=end_date).count() for
                     query in
                     queryset]
        nums = sum(nums_list) or 0
        return nums
...
class RecommendQuestion(object):
    ...
    def get_label_question(self, label):
        '''获取用户关注的话题下所有未回答过的问题'''
        questions = label.label.question_set.exclude(answer__author_id=self.user.uid)[:self.offset + self.limit]
        question_list = [question for question in questions]
        return question_list

    def get_all_question(self):
        '''获取所有问题中，用户未回答过的问题'''
        questions = Question.objects.exclude(answer__author_id=self.user.uid).order_by('-create_at')[
...
class RecentCreateContent(object):
    ...
    def recent_thinks(self):
        thinks = Idea.objects.filter(author=self.user).order_by('create_at')[:3]
        thinks_data = []
        for think in thinks:
            data = dict()
            data['id'] = think.id
    ...
    def get(self, request):
        ...
        if data_type == 'article':
            ...
            for article in articles:
                ...
                data_list.append(data)
        if data_type == 'think':
            thinks = Idea.objects.filter(create_at__gte=begin_da, create_at__lte=end_da, author=user)
            for think in thinks:
                data = dict()
                data['id'] = think.id
        ...
        if params_query.get('several_id'):
            ...
            for content in contents:
                ...
                if content_type == 'answer':
                    ...
                    user = UserProfile.objects.get(uid=answer.author_id)
                ...
                if content_type == 'article':
                    try:
                        ...
                        user = UserProfile.objects.get(uid=article.author_id)
                        user_data = {'avatar': user.avatar, 'nickname': user.nickname}
                        data['author_data'] = user_data
                        data['score'] = content.score
