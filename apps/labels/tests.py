...
class OneLabelViewDeleteTest(TestCase):
    ...
    def test_label_not_exist(self):
        path = reverse("labels:one_label", kwargs={"label_id": self.label.pk + 1})
        response = self.client.delete(path, **self.headers)
        data = response.json()
        self.assertEqual(data["code"], 0)

    def test_label_exist(self):
        path = reverse("labels:one_label", kwargs={"label_id": self.label.pk})
        response = self.client.delete(path, **self.headers)
        data = response.json()
        self.assertEqual(data["code"], 0)
...
class ChildLabelViewPostTest(TestCase):
    def setUp(self):
        ...
        self.path = reverse("labels:child", kwargs={"label_id": self.label1.pk})
    ...
    def test_no_parent(self):
        path = reverse("labels:child", kwargs={"label_id": self.label1.pk + self.label2.pk})
...
class ChildLabelViewDeleteTest(TestCase):
    def setUp(self):
        ...
        self.path = reverse("labels:child", kwargs={"label_id": self.parent.pk})
    ...
    def test_no_parent(self):
        path = reverse("labels:child", kwargs={"label_id": self.parent.pk + self.child.pk})
...
class LabelFollowViewPostTest(TestCase):
    def setUp(self):
        ...
        self.path = reverse("labels:follow", kwargs={"label_id": self.label.pk})
    ...
    def test_label_not_exist(self):
        path = reverse("labels:follow", kwargs={"label_id": self.label.pk + 1})
